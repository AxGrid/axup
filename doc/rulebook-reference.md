...
`deploy` invocation. It declares the project's name, top-level variables,
optional external dependencies, optional secrets layout, and two task lists:
`bootstrap:` (called by `deploy bootstrap`) and `deploy:` (called by
`deploy deploy`).
...
bootstrap:                            # tasks run by `deploy bootstrap`
...
`deploy` invocation even when fanned out to multiple hosts. The remote pulls
...
`deploy bootstrap` / `deploy deploy` reject the rulebook at parse time if:
...
my-project/
├── rulebook.yaml
├── deploy.lock                  # generated by `deploy deps tidy`
